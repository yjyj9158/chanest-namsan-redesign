...
export function PreferencesForm() {
  ...
  return (
    <main className="min-h-svh bg-cream px-6 py-12 text-charcoal">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {SCENT_OPTIONS.map((option) => (
              <ToggleCard
                ...
                onClick={() => setScent((cur) => (cur === option.id ? null : option.id))}
              />
            ))}
          </div>
        </section>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            {TEMPERATURE_OPTIONS.map((option) => (
              <ToggleCard
                ...
                onClick={() =>
                  setTemperature((cur) => (cur === option.id ? null : option.id))
                }
              />
            ))}
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
            {PARTY_OPTIONS.map((option) => (
              <ToggleCard
                ...
                onClick={() => setParty((cur) => (cur === option.id ? null : option.id))}
              />
            ))}
          </div>
        </section>
